...
TASC is the second CLI in this pack. It gives blind and low-vision architects programmatic control of Rhino site layouts via accessible text commands.

### Expanded Command Set (v0.1.0)

| Command | Purpose |
|---------|---------|
| `tasc site` | Define site boundary |
| `tasc grid` | Structural grid |
| `tasc zone` | Program zones (living, kitchen, etc.) |
| `tasc bay` | Structural bays with column grids |
| `tasc corridor` | Corridors within bays |
| `tasc void` | Courtyards/atriums within bays |
| `tasc label` | Text + Braille labels on zones/bays |
| `tasc undo` | Revert last command |
| `tasc remove` | Remove zones or bays |
| `tasc describe` | Full text description |
| `tasc list` | Short listing |
| `tasc export` | Export to .3dm, text, or PIAF PDF |

### Bay vs Zone

- **Zone**: A named program area (e.g. "living", "kitchen"). Freeform rectangle or polygon.
- **Bay**: A structural column grid (e.g. 6x3 at 24-foot spacing). Contains columns, optional corridors and voids.

Both can coexist on the same site. Use zones for spatial programming, bays for structural logic.

### Testing

146 tests passing across CLI, model, feedback, export, and DSL modules:
```bash
cd lib/tasc-core && python -m pytest tests/ -v
```

### Daniel-Mode CLAUDE.md

The `.claude/CLAUDE.md` file contains screen-reader-specific AI guidelines (plain text only, no markdown, TASC command priority over RhinoPython). It is loaded automatically by Claude Code when working in this repo.

### Full Documentation

See `lib/tasc-core/README.md` for complete CLI and DSL reference.

## Last Updated

**2026-02-22** - Added TASC structural commands (bays, corridors, voids, labels, undo). Updated PAI pack compliance. 146 tests passing.
